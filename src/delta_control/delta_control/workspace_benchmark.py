...
def load_points_file(path, unit):
    ...
    if path.suffix.lower() == ".json":
        ...
        if not isinstance(data, list):
            raise ValueError("JSON must be a point list or contain a 'points' list.")
        rows = data
    elif path.suffix.lower() == ".csv":
        with path.open(newline="", encoding="utf-8-sig") as stream:
            raw_rows = list(csv.reader(stream))
        if not raw_rows:
            raise ValueError("The points CSV is empty.")
        try:
            [float(value) for value in raw_rows[0][:3]]
            rows = raw_rows
        except (TypeError, ValueError):
            rows = raw_rows[1:]
    else:
        raise ValueError("Points file must use the .csv or .json extension.")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if len(values) < 3 or any(value is None for value in values):
            raise ValueError(f"Point {index} does not contain x, y and z.")
        try:
            points.append(tuple(float(value) * scale for value in values))
        except (TypeError, ValueError) as error:
            raise ValueError(f"Point {index} contains a non-numeric value.") from error

    if not points:
        raise ValueError("The points file does not contain any points.")
    return points


def build_points_path(solver, points):
    home = (solver.g.home_x, solver.g.home_y, solver.g.home_z)
    ...
    for index, point in enumerate(points, start=1):
        ...
        except DeltaIKError as error:
            point_mm = tuple(1000.0 * value for value in point)
            raise ValueError(
                f"Point {index} {point_mm} mm is outside the workspace."
            ) from error

    for index, (start, end) in enumerate(zip(path, path[1:]), start=1):
        if not line_is_reachable(solver, start, end):
            raise ValueError(
                f"Segment {index} is not fully reachable; add intermediate points."
            )
    return path
...
def parse_args():
    ...
    parser.add_argument(
        "--points-file",
        type=Path,
        help="CSV or JSON file containing x, y, z diagnostic points.",
    )
    parser.add_argument(
        "--points-unit",
        choices=("mm", "m"),
        default="mm",
        help="Coordinate unit used by --points-file (default: mm).",
    )
    parser.add_argument(
        "--preserve-voxels",
        action="store_true",
...
def main():
    ...
    mesh = None
    if args.points_file:
        points = load_points_file(args.points_file, args.points_unit)
        benchmark_path = build_points_path(solver, points)
        ...
        source_count = len(mesh.scan_path)
    ...
    print(
        f"Benchmark path: {source_count} source points -> "
    ...
    result.update(
        {
            ...
            "path_source": (
                str(args.points_file) if args.points_file else "workspace"
            ),
            "path_source_points": source_count,
            "path_voxels": len(mesh.scan_path) if mesh else None,
